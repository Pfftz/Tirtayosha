class Agent:
    def __init__(self):
        self._wumpusWorld = [
            ['', '', '', ''],  # Rooms [1,1] to [4,1]
            ['', 'P', '', ''],  # Rooms [1,2] to [4,2]
            ['W', '', 'G', ''],  # Rooms [1,3] to [4,3], Gold at [3,3]
            ['', '', '', ''],  # Rooms [1,4] to [4,4]
    ...
    def _CheckForGold(self):
        ww = self._wumpusWorld
        i, j = self._FindIndicesForLocation(self._curLoc)
        if 'G' in ww[i][j]:
            self._hasGold = True
            print('Agent has found the GOLD!')
        return self._hasGold
    ...
    def TakeAction(self, action):
        ...
        if self._isAlive == False:
            print('Action cannot be performed. Agent is DEAD. Location:{0}'.format(
                self._curLoc))
            return False
        if self._hasExited == True:
            print('Action cannot be performed. Agent has exited the Wumpus world.'.format(
                self._curLoc))
            return False
        ...
        for v, inc in zip(self._curLoc, move):
            ...
            if z < 1 or z > 4:
                print('Move out of bounds. Action not taken.')
                return False
            newLoc.append(z)
        self._curLoc = newLoc
        print('Action Taken: {0}, Current Location {1}'.format(
            action, self._curLoc))
    ...
    # This function perceives the current location.
    def PerceiveCurrentLocation(self):
        ...
        ww = self._wumpusWorld
        if self._isAlive == False:
            print('Agent cannot perceive. Agent is DEAD. Location:{0}'.format(
                self._curLoc))
            return [None, None]
        if self._hasExited == True:
            print('Agent cannot perceive. Agent has exited the Wumpus World.'.format(
                self._curLoc))
            return [None, None]
